...
def PlaySong(url, title, img):
	...
	try:		iitems=urllib.unquote_plus(re.compile('<param name="FlashVars" value="mp3=(http.+?)&').findall(html)[0])
	except:	iitems=None
	...
	play.play(pug)
	return
#<param name="FlashVars" value="mp3=http%3A%2F%2Fs12.searchmp3.mobi%2Fd%2F--ko0m9oyy_H7Rpl858O4Q%2F1377571712%2Fmp3%2Fs%2Fselena-gomez%2FSelena+Gomez+-+When+You+Ready+Come+And+Get+It.mp3%3Ff%3DSelena%252520Gomez%252520-%252520When%252520You%252520Ready%252520Come%252520and%252520Get%252520It.mp3&amp;width=460&amp;height=41&amp;loop=1&amp;volume=100&amp;showvolume=1&amp;showstop=0&amp;bgcolor1=4E535A&amp;bgcolor2=4E535A&amp;slidercolor1=F6D80B&amp;slidercolor2=F6D80B&amp;buttoncolor=F6D80B" />
...
def listItems(section=_default_section_, url='', startPage='1', numOfPages='1', genre='', year='', stitle='', season='', episode='', html='', chck=''): # List: Movies or TV Shows
	...
	html=''
	url2=url+'?page='+str(startPage)
	WhereAmI('@ Page: %s' % url2) #['pageno']
	try: html=net.http_GET(url2).content
	except: 
		try: html=getURL(url2)
		except: 
			try: html=getURLr(url2,_domain_url)
			except: html=''
	if (html=='') or (html=='none') or (html==None): deb('Error','Problem with page'); deadNote('Results:  '+section,'No results were found.'); return
	deb('Length of HTML',str(len(html)))
	try:		NextPageNo=re.compile('<a href=".page=(\d+)" class="more nav-page">&gt;</a>').findall(html)[0]
	except:	NextPageNo=None
	if (NextPageNo is not None) and (NextPageNo is not '') and (NextPageNo is not startPage):
		_addon.add_directory({'section':section,'mode':'GetTitles','url':url,'pageno':NextPageNo}, {'title':cFL_('Next Page  ('+cFL(NextPageNo,ps('cFL_color2'))+')',ps('cFL_color'))}, img=art('icon-next','.png'), fanart=_artFanart)
	html=messupText(html,True,True,True,False)
	#print html
	#html=ParseDescription(html); html=remove_accents(html) #if (_debugging==True): print html
	s='<li\s+itemscope\s+itemtype=".*?"\s+class=\'fade_rtl\'\s*>.+?</li>'
	try:		iitems=re.compile(s, re.MULTILINE | re.DOTALL).findall(html)
	except:	iitems=None
	if (iitems is not None):
		ItemCount=len(iitems)
		deb('Number of Items',str(ItemCount))
		for item in iitems:
			contextMenuItems=[]; labs={}; labs2={}; 
			try:		url=re.compile('<meta\s+itemprop="url"\s+content="(.+?)"\s*/>').findall(item)[0]
			except:	url=None
			if (url is not None) and (url is not ''):
				try: 		labs['Artist']=re.compile('<meta\s+itemprop="byArtist"\s+content="(.+?)"\s*/>').findall(item)[0]
				except:	labs['Artist']='Unknown'
				try: 		labs['SongTitle']=re.compile('<meta\s+itemprop="name"\s+content="(.+?)"\s*/>').findall(item)[0]
				except:	labs['SongTitle']='Unknown'
				try: 		labs['Album']=re.compile('<meta\s+itemprop="inAlbum"\s+content="(.+?)"\s*/>').findall(item)[0]
				except:	labs['Album']='Unknown'
				try: 		labs['duration']=re.compile('<meta\s+itemprop="duration"\s+content="(.+?)"\s*/>').findall(item)[0]
				except:	labs['duration']='Unknown'
				try: 		img=re.compile('<img src="(http://.+?.jpg)"').findall(item)[0]
				except:	img=_artIcon
				labs['title'] =labs['Artist']
				labs2['Artist']=labs['Artist']; labs2['SongTitle']=labs['SongTitle']; labs2['Album']=labs['Album']; labs2['duration']=labs['duration']
				if (labs['Album'] is not 'Unknown album') and (labs['Album'] is not 'Unknown'): labs['title']+=' - '+labs['Album']
				if (labs['Album'] is not 'Unknown'): labs['title']+=' - '+labs['SongTitle']
				labs2['title']=cFL_('Watch:  '+labs['title'],ps('cFL_color2')); 
				labs['title'] =cFL_('Listen:  '+labs['title'],ps('cFL_color')); 
				pTitle=labs['Artist']+' - '+labs['Album']+' - '+labs['SongTitle']
				title1=labs['Artist']+' - '+labs['Album']; title2=cFL(labs['SongTitle'],ps('cFL_color3'))
				labs['title']=cFL_('Listen:  '+title1,ps('cFL_color')); labs2['title']=cFL_('Watch:  - '+title2,ps('cFL_color2'))
				parsPS={'mode': 'PlaySong' , 'section': section, 'url': url, 'img': img, 'title': pTitle }
				parsPV={'mode': 'PlayVideo', 'section': section, 'url': url, 'img': img, 'title': pTitle }
				_addon.add_directory(parsPS, labs, img=img, fanart=_artFanart, contextmenu_items=contextMenuItems, total_items=ItemCount)
				_addon.add_directory(parsPV, labs2, img=img, fanart=_artFanart, contextmenu_items=contextMenuItems, total_items=ItemCount)
...
def Menu_MainMenu(): #The Main Menu
	...
	_addon.add_directory({'mode': 'GetTitles', 'section': 'music', 'url': 'http://searchmp3.mobi/top-mp3-downloads-today.xhtml'}, {'title': cFL_('Top Downloaded (Today)',ps('cFL_color'))}, fanart=_artFanart)
	_addon.add_directory({'mode': 'GetTitles', 'section': 'music', 'url': 'http://searchmp3.mobi/top-mp3-downloads-this-week.xhtml'}, {'title': cFL_('Top Downloaded (This Week)',ps('cFL_color'))}, fanart=_artFanart)
	_addon.add_directory({'mode': 'GetTitles', 'section': 'music', 'url': 'http://searchmp3.mobi/top-mp3-downloads-this-month.xhtml'}, {'title': cFL_('Top Downloaded (This Month)',ps('cFL_color'))}, fanart=_artFanart)
	_addon.add_directory({'mode': 'GetTitles', 'section': 'music', 'url': 'http://searchmp3.mobi/top-mp3-downloads-all-time.xhtml'}, {'title': cFL_('Top Downloaded (All Time)',ps('cFL_color'))}, fanart=_artFanart)
	_addon.add_directory({'mode': 'GetTitles', 'section': 'music', 'url': 'http://searchmp3.mobi/top-mp3-rated-today.xhtml'}, {'title': cFL_('Top Rated (Today)',ps('cFL_color'))}, fanart=_artFanart)
	_addon.add_directory({'mode': 'GetTitles', 'section': 'music', 'url': 'http://searchmp3.mobi/top-mp3-rated-this-week.xhtml'}, {'title': cFL_('Top Rated (This Week)',ps('cFL_color'))}, fanart=_artFanart)
	...
	##_addon.add_directory({'section': 'music', 'mode': 'Search', 				'pageno': '1', 'pagecount': addst('pages')},			{'title':  cFL('S',ps('cFL_color'))+'earch'}, 						fanart=_artFanart,img=art('icon-search'))
	...
	_addon.add_directory({'mode': 'YoutubeSettings'}, {'title':  cFL_('Youtube Settings',ps('cFL_color3'))},is_folder=False		,img=ps('art_youtube')	,fanart=_artFanart)
	_addon.add_directory({'mode': 'Settings'}, 				 {'title':  cFL_('Plugin Settings',ps('cFL_color2'))}			,is_folder=False		,img=_artDead							,fanart=_artFanart)
